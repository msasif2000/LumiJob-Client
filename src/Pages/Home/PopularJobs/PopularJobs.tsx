...
const PopularJobs = () => {
  ...
  const slicedSectors = sectors.slice(1, 7);
  ...
  const filterJob = popularJobs?.filter(
    (job) => job.sectorType === sectors[tabIndex+1]?.sectorType
  );
          ...
          {sectors?.map((sector: any) => (
            <TabPanel key={sector._id}>
              {loading ? (
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4 xl:gap-6 my-6 lg:my-20">
                  {slicedJobs?.map((job) => (
